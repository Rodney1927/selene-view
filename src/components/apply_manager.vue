          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div class="link-layout">
                <div>
                  <el-link type="primary" @click="agreeApplyPosition(scope.row)">同意</el-link>
                </div>
                <div>
                  <el-link type="primary" @click="refuseApplyPosition(scope.row)">拒绝</el-link>
                </div>
              </div>
            </template>
          </el-table-column>
  ...
  methods:{
    refuseApplyPosition(id){
      this.$prompt('请输入拒绝原因', '拒绝', {
        ...
        let p = new FormData()
        p.append('id',id)
        p.append('remark',value)
        this.$axios({
          method : "POST",
          url: "/selene/apply/refuse", //todo
          data : p
        }).then(res=>{
          const data = res.data
          if (data.code !== '200'){
    ...
    agreeApplyPosition(id){
      this.$prompt('请输入面试时间', '同意', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        inputValidator: this.checkMessageBox,
        inputErrorMessage: '面试时间不能为空'
      }).then(({value}) => {
        let p = new FormData()
        p.append('id',id)
        p.append('date',value)
...
<style src="../assets/css/apply_manager.css" scoped/>
