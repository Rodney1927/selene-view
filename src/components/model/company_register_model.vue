  ...
  <div class="modal-backdrop" >
    <div class="modal">
      <div class="register-line-first">
        <div class="register-name">{{'账号:'}}</div>
        <div class="register-right">
          <el-input v-model="registerAccount.username" placeholder="请输入账号" style="width: 260px" clearable
      ...
      <div class="register-line">
        <div class="register-name">{{'企业名称:'}}</div>
        <div class="register-right">
          <el-input v-model="registerAccount.name" placeholder="请输入企业名称" style="width: 260px" clearable
      ...
      <div class="register-line">
        ...
        <div class="register-right">
          <el-input v-model="registerAccount.email" placeholder="请输入企业邮箱" style="width: 260px" clearable
                    @input="validRegisterEmail"></el-input>
        </div>
      </div>
      <div class="register-msg">{{registerEmailMsg}}</div>
      <div class="register-line">
        <div class="register-name">{{'电话:'}}</div>
        <div class="register-right">
          <el-input v-model="registerAccount.mobile" placeholder="请输入企业电话" style="width: 260px" clearable
      ...
      <div class="modal-footer">
        <button type="button" class="btn-close" @click="closeCompanyRegisterModel">关闭</button>
        <button type="button" class="btn-confirm" @click="companyRegister" :disabled="registerButtonFlag">确认</button>
      </div>
    </div>
  </div>
</template>
...
import {isEmail, isMobile, isPassword, isUsername} from "@/common/util/validate";

export default {
name: "company_register_model",
  data(){
    return{
      registerAccount: {
        username: '',
  ...
  methods:{
    closeCompanyRegisterModel(){
      this.$emit("closeCompanyRegisterModel")
      this.clearData()
    },
    clearData(){
    ...
    companyRegister(){
      ...
      this.$axios({
        ...
        this.closeCompanyRegisterModel()
      })
    },
    validRegisterFlag(){
    ...
    validRegisterUsername(){
      if (!isUsername(this.registerAccount.username)){
        this.registerUsernameMsg = '账号长度6-12位，英文开头可包含数字英文下划线'
        this.registerUsernameFlag = false
      }
      else {
        this.registerUsernameMsg = ''
...
<style src="../../assets/css/company_register_model.css" scoped/>
